...
- **Align crossword name and score rows** with the `avg` / `pb` centre divider: both rows render a centre `|`, the name/score divider line uses a `+` at that centre, and each side sits exactly 1 character from the centre divider. Emoji-grid scoreboards keep their existing spacing.
...
- **Modified renderers**: `MiniCrosswordScoreboard`, `MidiCrosswordScoreboard`, `MainCrosswordScoreboard` (in `nyt-scorebot-service`) — each gains an `avg`/`pb` row, and Main gains the day-of-week header plus the omitted-empty-flags-row behaviour. The shared `ScoreboardRenderer` uses an opt-in crossword layout path so all three crossword renderers stay consistent without changing Wordle / Connections / Strands spacing.
